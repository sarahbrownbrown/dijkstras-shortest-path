...
/**
 * The Graph class implements various operations on the graph data
 * structure, including Dijkstra's Shortest Path algorithm. 
 * 
 * @author sarahbrown
 *
...
public class Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {     
        ...
        LinkedList<Vertex> visited = new LinkedList<Vertex>(); // stores already visited vertices
        ...
        while(!pq.isEmpty()) {
            currentPath = (Graph<T>.Path) pq.peek();
            Vertex currentVertex = currentPath.end;
            visited.add(currentVertex); // add current vertex to visited
            pq.remove(); // remove head of queue
            if (currentVertex.data.equals(end)) {
                break;
            }                     
            for (int i = 0; i < currentVertex.edgesLeaving.size(); i++) {
                if (visited.contains(currentVertex.edgesLeaving.get(i).target)) {
                    continue;  // if vertex already visited go to next edge
                }
                Path copy = new Path(currentPath);
                copy.extend(currentVertex.edgesLeaving.get(i));
                pq.add(copy);
            }                      
        }             
        return currentPath;
